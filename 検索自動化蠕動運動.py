...
search.send_keys('dysbiosis Peristalsis japan research')             # 検索ワードを送信する
search.submit()                             # 検索を実行
...
# タイトルリストをテキストに保存
with open('title_蠕動運動.txt', mode='a', encoding='utf-8') as f:
    f.write("\n".join(title))
# URLリストをテキストに保存
with open('link_蠕動運動.txt', mode='a', encoding='utf-8') as f:
    f.write("\n".join(link))
